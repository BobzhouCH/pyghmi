...
class Command(object):
    ...
    def _get_zero_fru(self):
        ...
        if (device_id['Manufacturer ID'] == 19046 and device_id['Product ID'] == 13616):
            # Currently SD350 FRU UUID is synchronized with the Device UUID. Need to change to System UUID in future.
            # Since the IPMI get device uuid matches SMBIOS, no need to decode it. 
            guiddata = self.raw_command(netfn=6, command=0x8)
            ...
            if 'error' not in guiddata:
                if (device_id['Manufacturer ID'] == 19046 and
                    (device_id['Product ID'] == 1182 or device_id['Product ID'] == 1184)):
                    # The manufacturer (Asrockrack) of RS160/TS460 matches SMBIOS
                    # to IPMI get system uuid return data, no need to decode it.
                    zerofru['UUID'] = pygutil.decode_wireformat_uuid(
                        guiddata['data'], True)
                else:
                    zerofru['UUID'] = pygutil.decode_wireformat_uuid(
    ...
    def get_net_configuration(self, channel=None):
        ...
        return retdata, self._oem.get_extra_net_configuration()
    ...
    def set_user_access(self, uid, channel=None, callback=False,
        ...
        # Set kvm privileges as enabled for users with administrator level.
    ...
    def set_user_password(self, uid, mode='set_password', password=None):
        ...
        else: # by chenjiang1, for Huawei reset user password
            password = b'\x00' * 16
            data.extend([ord(x) for x in password])
        try:
    ...
    def user_delete(self, uid, channel=None):
        ...
        try:
            ...
            try:
                ...
                self.set_user_name(uid, '(Empty User)')
        return True
    ...
    def get_ris_configuration_parameters(self, media_type_id):
        """Get RIS configuration parameters"""
        self.oem_init()
        return self._oem.get_ris_configuration_parameters(media_type_id)

    def set_ris_configuration_parameter(self, media_type_id, param_id, value):
        """Set RIS configuration parameter"""
        self.oem_init()
        return self._oem.set_ris_configuration_parameter(media_type_id, param_id, value)

    def set_ris_redirection_state(self, image_type, value):
        """Set RIS redirection state"""
        self.oem_init()
        return self._oem.set_ris_redirection_state(image_type, value)

    def set_media_redirection_state(self, image_type, image_name, value):
        """Set Media redirection state"""
        self.oem_init()
        return self._oem.set_media_redirection_state(image_type, image_name, value)
