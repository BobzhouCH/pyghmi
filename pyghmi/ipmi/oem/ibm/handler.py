...
class OEMHandler(generic.OEMHandler):
            ...
            for item in sdr_list:
                ...
                if ord(item[8]) == 0xed:
                    try:
                        ...
                        #get the led status
                        rsp = self.ipmicmd.xraw_command(netfn=0x3A, command=0xc0,
                                                        data=led_identifier)
                    except pygexc.IpmiException:
                        continue  #ignore LEDs we can't retrieve
                    
                    state = led_state.get(ord(rsp['data'][1]), led_state_default)
                    color = led_color.get(ord(rsp['data'][0]), led_color_default)
                    location = led_location.get(ord(item[14]), led_location_default)
                    yield (led_name, {'status': state, 'color': color, 'location': location})
    ...
    def process_fru(self, fru):
        ...
        if self.has_imm:
            ...
            vals = fru.values()
            for val in vals:
                if isinstance(val, basestring) and re.match(r'.*(cpu|processor).*', val, re.IGNORECASE):
                    return None
            return fru
        else:
            fru['oem_parser'] = None
                    ...
                    for oem_string in item['OEM Strings']:
                        ...
                        for dsa in ('DSYT', 'DSAL', 'DSY1'):
                            ...
                            if dsa_idx != -1:
                                break
                        if imm_idx != -1:
                            version = oem_string[imm_idx : imm_idx+7] + '-' + oem_string[imm_idx-7 : imm_idx-3]
                            fw_info.append(('IMM', {'version': version}))
                        if dsa_idx != -1:
                            version = oem_string[dsa_idx : dsa_idx+7] + '-' + oem_string[dsa_idx-7 : dsa_idx-3]
    ...
    def _get_oem_sdr_list(self):
        if self.has_imm:
            ...
            while True:
                try:
                    rsp = self.ipmicmd.xraw_command(netfn=0x0a, command=0x23, data=data)
                except pygexc.IpmiException, ie:
                    if ie.ipmicode in (202, 204):
                        #if the number of requested bytes cannot be returned, reduce the size 
                        data[5] = data[5] - 2
                        continue
                    raise
                ...
                data[5] = 0x20
                if (ord(rsp['data'][0]) == 0xff and ord(rsp['data'][1]) == 0xff):
                    last_sdr = True
            return sdr_list
    ...
    def get_extra_net_configuration(self):
        ipv6_addr = self.ipmicmd.xraw_command(netfn=0x0c, command=0x02,
            data=(0x01, 0xc8, 0x00, 0x00))["data"][2:]
        if not ipv6_addr:
            return '::'
