...
class FFDCFetcher(object):
    ...
    def check_ffdc_status(self, wait=True):
        ...
        """
        try:
            status_rep = self.ipmicmd.xraw_command(netfn=0x3a, command=0x4d, data=[1])
    ...
    def fetch_datastore_name(self, name, filename, waitforready=False):
        ...
        if status == 0x01:
            ...
            checkdssize = self.ipmicmd.raw_command(netfn=0x2e, command=0x90, data=rqdata)
        ...
        #get the ffdc handle
        rqdata = [ 0x4d, 0x4f, 0x0, 1, 1, 0, ] + checkdssize['data'][3:7] + [0, 30] + dsname
        opends = self.ipmicmd.raw_command(netfn=0x2e, command=0x90, data=rqdata)
        ...
        while chunksize > 0:
            ...
            if chunksize > 0:
                currchunk = list(struct.unpack('2B', struct.pack('<H', chunksize)))
                rqdata = [0x4d, 0x4f, 0x0, 2] + dshandle + curroff +  currchunk
                readit = self.ipmicmd.raw_command(netfn=0x2e, command=0x90, data=rqdata)
                if readit['code'] != 0:
                    return False
                dsout.write(struct.pack('%dB' % len(readit['data'][5:]), *readit['data'][5:]))
    ...
    def fetch_ffdc_file(self, filename):
        ...
        if status == 0x02:
            ...
            begindump = self.ipmicmd.raw_command(netfn=0x3a, command=0x49, data=[1])
            if 'error' in begindump:
                print begindump['error']
                return False
            self.ipmicmd.wait_for_rsp(5)
        #fetch the ffdc
        return self.fetch_datastore_name(b'ffdc.tgz', filename, waitforready=True)
