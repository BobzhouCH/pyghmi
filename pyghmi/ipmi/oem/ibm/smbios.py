...
oem_smbios_cmds = {
    "query" : {
        "netfn": 0x2e,
        "command": 0x90,
        "data": [0x4d, 0x4f, 0x00, 0x06, 0x73, 0x6d, 0x62, 0x69, 0x6f, 0x73, 0x2d, 0x74, 0x61, 0x62, 0x6c, 0x65]},
    "open" : {
        "netfn": 0x2e,
        "command": 0x90,
        "data": [0x4d, 0x4f, 0x00, 0x01, 0x01, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x73, 0x6d, 0x62, 0x69, 0x6f, 0x73, 0x2d, 0x74, 0x61, 0x62, 0x6c, 0x65]},
    "read" : {
        "netfn": 0x2e,
        "command": 0x90,
        "data": [0x4d, 0x4f, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xe0, 0x00]},
    "close" : {
        "netfn": 0x2e,
        "command": 0x90,
...
def get_smbios_info(ipmicmd):
    ...
    smbios_table_size = ord(rsp["data"][3]) | ord(rsp["data"][4]) << 8 | ord(rsp["data"][5]) << 16 | ord(rsp["data"][6]) << 24
...
def type1(input_file):
    ...
    try:
        ...
        uuid = uuid_ary[3] + uuid_ary[2] + uuid_ary[1] + uuid_ary[0] + uuid_ary[5] + uuid_ary[4] + uuid_ary[7] + uuid_ary[6] + uuid_ary[8] + uuid_ary[9] + uuid_ary[10] + uuid_ary[11] + uuid_ary[12] + uuid_ary[13] + uuid_ary[14] + uuid_ary[15]
        return {
            "Type" : "System",
...
def type4(input_file):
    ...
    """       
    try:
        socket_designation, processor_family_name, processor_manufacturer, processor_version, speed, status, serial_number, core_count, core_enabled, thread_count = '', '', '', '', '', '', '', 0, 0, 0
        input_file.read(2)
