...
class Energy(object):
    ...
    def has_subsystem_metering(self):
        try:
            ...
            fw_cap = self.read_element_register(self.handle, aem_cmds["fw power cap reg"])
            if ord(fw_cap[0]) & 0x80:
                #get the extended firmware capability register
                fw_ext = self.read_element_register(self.handle, aem_cmds["extend fw cap reg"])
    ...
    def read_snapshot_buffer(self):
        if self.has_subsystem_metering:
            length, config = self.read_element_configuration(aem_cmds["element config"], self.handle)
            ...
            while offset < total_size:
                ...
                if prior_buffer_tag and not buffer_tag == prior_buffer_tag:
                    #reset the offset to beginning if the snapshot is updated in the reading
                    offset = 0
                    snapshot_buffer = []
                    buffer_tag, prior_buffer_tag = None, None
                else:     
