    ...
    def decode_sensor_reading(self, reading):
        ...
        if discrete:
            for state in range(8):
                if reading[2] & (0b1 << state) & self.assertion_reading_mask[0]:
            ...
            if len(reading) > 3:
                for state in range(7):
                    if reading[3] & (0b1 << state) & self.assertion_reading_mask[1]:
            ...
            rsp = self.ipmicmd.xraw_command(netfn=4, command=0x2b,
                                       data=(self.sensor_number,))
            rsp['data'] = bytearray(rsp['data'])
            for offset in range(8):
                if rsp['data'][1] & (0b1 << offset) & self.assertion_reading_mask[0]:
                    output['state_ids'].append(self.assert_trap_value(offset))
            if len(rsp['data']) >= 3:
                for offset in range(4):
                    if rsp['data'][2] & (0b1 << offset) & self.assertion_reading_mask[1]:
                        output['state_ids'].append(self.assert_trap_value(offset+8))
